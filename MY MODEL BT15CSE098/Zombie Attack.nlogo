...
to setup
  ...
  create-HealthyPeople 100  ;; create the HealthyPeople, then initialize their variables
  [
    set color 137
  ...
  create-Zombies 15  ;; create the HealthyPeople, then initialize their variables
  [
    set color 66
    set size 1.5  ;; easier to see
    setxy random-xcor random-ycor
...
to catchPerson ;Procedure for Zombies
  ...
  if prey != nobody and random 100 < 20 ;If the zombie has found a person and the infection rate is within range, the person becomes a zombie
    [ ask prey [ set breed Zombies set color 66 ] ]
end

to catchHunter ;Procedure for Zombies
  let prey one-of Hunters-here
  if prey != nobody and random 100 < 20 ;If the zombie has found a hunter and the infection rate is within range, the hunter becomes a zombie
    [ ask prey [ set breed Zombies set color 66 ] ]
end

to catchZombies
  let prey one-of Zombies
  if prey != nobody and random 100 < 1
    [ ask prey [ die ] ]
end
@#$#@#$#@
GRAPHICS-WINDOW
