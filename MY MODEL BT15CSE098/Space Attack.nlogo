...
to setup
  ...
  ;Initialize army in world
  set-default-shape army "airplane"
  create-army 100  ;; create the army, then initialize their variables
  [
    set color 137
  ...
  ;Initialize attackers in world
  set-default-shape attackers "default"
  create-attackers 15  ;; create the attackers, then initialize their variables
  [
    set color 66
    set size 1.5  ;; easier to see
    setxy random-xcor random-ycor
...
to go
    if not any? army [ stop ] ;stop the program if there are only safe and attackers left
    if not any? attackers [ stop ]
    ask army [ ;Run procedures for army
      move
      checkSafety
    ]
    ask attackers [ ;Run procedures for attackers
      move
      catchPerson

    ]
    ask safe [ ;Run procedures for immune people
      move
    ]
...
to move ;Movement for army, attackers
  rt random 360
  fd 1
end

to checkSafety ;Procedure for army
  if pcolor = 7 or pcolor = 15 ;If the army are at the government facility, they become safe
    [
      set breed safe
      set shape "airplane"
      set color 47
...
to catchPerson ;Procedure for attackers
  let prey one-of army-here
  if prey != nobody and random 100 < 20 ;If the attackers has found a person and the attack rate is within range, the army dies
    [ ask prey [ die ] ]
end
@#$#@#$#@
GRAPHICS-WINDOW
